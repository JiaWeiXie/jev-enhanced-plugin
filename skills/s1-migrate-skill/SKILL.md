...
- Add one section, `## Jev <what it judges> (advisory)`, placed at the step where the judgment belongs. It contains, in this order: `Read ../jev-advisory.md first`; the `job.json` state shape as a JSON block matching the pack header field for field; the command `node "${CLAUDE_PLUGIN_ROOT}/src/cli.mjs" <pack> --state job.json --json`; how to read the result (`mode`, `unknown`, `leans yes` / `leans no` / `unclear`); the hard limits that apply to this skill; what is sent to the third party and the instruction to ask the user before the first live run; and the exit 3 behavior.
